...
@ManagedBean()
@ViewScoped
public class Vista_tipo_informacion extends Vista_campostxt implements Serializable {
    ...
    public void eliminar(String cod) {
        ...
        if (codigo_oracle_relacionado.equals(acciones.getmensajei().substring(0, 9))) {
            ...
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, acciones.getmensajei(), ""));
            rev.registro_actividad(getMostrarC(), "tipo_informacion", "eliminar", txtcodigomod);
            limpiar();
        }
    }

    public void modificar() {
        if (valida.campos2(txtcodigomod, txtnombresmod) == false) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, mensaje_requerir_datos, ""));
        } else {
            acciones.actualizar(txtcodigomod, txtnombresmod.toUpperCase());
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, acciones.getmensajei(), ""));
            rev.registro_actividad(getMostrarC(), "tipo_informacion", "modificar", txtcodigomod + txtnombresmod);
            limpiar();
        }
    }
    ...
    public ArrayList<entiv_tipo_informacion> getLista1() {
        ArrayList listae = new ArrayList();
        ...
        try {
            ...
            while (v.crs.next()) {
                objart = new entiv_tipo_informacion();
                objart.setCodigo(v.crs.getString("ti_codigo"));
                objart.setNombre(v.crs.getString("ti_descripcion"));
                listae.add(objart);
            }
        } catch (Exception e) {
            System.out.println("Error al recuperar datos" + e.getMessage());
    ...
    public void seleccionLista(String cod, String nombre) throws ParseException {
        this.txtcodigomod = cod;
        this.txtnombresmod = nombre;
    }
